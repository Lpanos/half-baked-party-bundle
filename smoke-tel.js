// Telephone smoke: 1 host + N players. Each player auto-submits the moment
// they receive `tel_your_turn`. Last player auto-guesses on `tel_guess_phase`.
// Everyone votes for the first chain shown on `tel_vote_phase`.
//
...
(async () => {
  ...
  const phrases = [
    ...
    'three retired pirates argue about brunch',
    'a polite robot tries running a bakery',
    'octopuses unionize against the local aquarium',
    'a soup convention attended only by sad spoons',
    'a parade led by an extremely small horse',
    'a doctor whose only patients are houseplants',
    'an alien tries to understand traffic court',
    'a town where everyone whispers everything always',
    'a confused chef hosts a gardening podcast',
    'a wedding where the cake is the officiant',
    'a courier delivering single grapes one at a time'
  ];
  ...
  for (let i = 0; i < N; i++) {
    ...
    const state = {
      sock, name: `P${i + 1}`, myId: '', writes: 0, guesses: 0, gotEnd: false, finalScore: 0
    };
    sock.on('joined', ({ yourId }) => { state.myId = yourId; });
    sock.on('error', ({ message }) => errors.push([state.name, message]));

    sock.on('tel_your_turn', ({ wordLimit, previousText }) => {
      const phrase = phrases[(i + state.writes) % phrases.length];
      const text = phrase.split(/\s+/).slice(0, wordLimit).join(' ');
      state.writes++;
      sock.emit('tel_submit_link', { text });
    });
    sock.on('tel_guess_phase', ({ wordLimit }) => {
      state.guesses++;
      sock.emit('tel_submit_guess', { text: 'something about cats and pirates' });
    });
    sock.on('tel_vote_phase', ({ chains }) => {
      // Every player votes for chain 0.
  ...
  log('  total writes per player:', players.map(p => `${p.name}=${p.writes}`).join(', '));
  log('  total guesses per player:', players.map(p => `${p.name}=${p.guesses}`).join(', '));
